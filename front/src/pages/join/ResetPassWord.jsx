...
const ResetPassWord = () => {
    ...
    const handleResetPassword = async () => {
        if (resetPassword.new_pw !== resetPassword.new_pw_check) {
            // 비밀번호 확인 불일치
            setWarning({
                ...warning,
                new: {
            ...
            return;
        } else if (resetPassword.present_pw === '') {
            setWarning({
                new: {
                    show: false,
                    msg: '',
            ...
            return;
        } else if (resetPassword.new_pw === '' || resetPassword.new_pw_check === '') {
            setWarning({
                present: {
                    show: false,
                    msg: '',
        ...
        try {
            const resp = await api.post('/myInfo/checkOldPassword', {
                ...
                oldPassword: resetPassword.present_pw,
            });
            ...
            const res = await api.post('/myInfo/password', {
                userId: String(user.userId),
                passwordNo: resetPassword.new_pw,
            });
    ...
    return (
        ...
        >
            <div>
                ...
                <ul>
                    <li className={resetPw.redText}>- 다른 아이디/사이트에서 사용한 적 없는 비밀번호</li>
                    <li className={resetPw.redText}>- 이전에 사용한 적 없는 비밀번호가 안전합니다.</li>
                </ul>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column' }} className={resetPw.topInput}>
                <Input
                    ...
                    setInputsState={setResetPassword}
                    className={resetPw.redPlaceholder} // 추가된 부분
                />
            ...
            <BottomButtons
                childrens={['확인', '취소']}
                handlers={[() => handleResetPassword(), () => handleCancel()]}
            />
            {/* <div className={resetPw.button}>
                <Button width="full" type="main" handler={() => handleResetPassword()}>
                    확인
                </Button>
